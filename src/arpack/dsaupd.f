...
c        if (which .ne. 'LM' .and.
c    &       which .ne. 'SM' .and.
c    &       which .ne. 'LA' .and.
c    &       which .ne. 'SA' .and.
c    &       which .ne. 'BE')                   ierr = -5
c        if (bmat .ne. 'I' .and. bmat .ne. 'G') ierr = -6
c
         if (lworkl .lt. ncv**2 + 8*ncv)        ierr = -7
         if (mode .lt. 1 .or. mode .gt. 5) then
                                                ierr = -10
c        else if (mode .eq. 1 .and. bmat .eq. 'G') then
c                                               ierr = -11
         else if (ishift .lt. 0 .or. ishift .gt. 1) then
                                                ierr = -12
c        else if (nev .eq. 1 .and. which .eq. 'BE') then
c                                               ierr = -13
         end if
c 
c        %------------%
