            ...
            MOLINFO * minfo)
{
    ...
    for (;;) {
        ...
        sscanf(line, "%s%s%s%s%s%s%s", tmpchar1, tmpchar2, tmpchar3, tmpchar4, tmpchar5,
               tmpchar6, tmpchar7);
        if (strcmp(tmpchar1, "Total") == 0 && strcmp(tmpchar2, "Mulliken") == 0
            && strcmp(tmpchar3, "Charge") == 0 && strcmp(tmpchar4, "=") == 0) {
            sscanf(line, "%s%s%s%s%lf", tmpchar1, tmpchar2, tmpchar3, tmpchar4,
                   &(*minfo).dcharge);
        ...
        if (flag == 1 && strcmp("QM_NO.", tmpchar2) == 0
            && strcmp("MM_NO.", tmpchar3) == 0 && strcmp("ATOM", tmpchar4) == 0
            && strcmp("X", tmpchar5) == 0 && strcmp("Y", tmpchar6) == 0
            && strcmp("Z", tmpchar7) == 0) {
            flag = 2;
            continue;
        }
        ...
        if (flag == 2) {
            if (overflow_flag == 0) {
                sscanf(line, "%s%d%d%s%lf%lf%lf", tmpchar1, &tmpint1, &tmpint2, tmpchar2,
...
void wsqmout(char *filename, int atomnum, ATOM * atom, CONTROLINFO cinfo, MOLINFO minfo)
{
