/* CSD */
int rcsd(char *filename, int *atomnum, ATOM atom[], CONTROLINFO cinfo, MOLINFO minfo)
{
    int i;
    int index = 0;
    int overflow_flag = 0;
...
/*       printf("\nFinished reading %s file.", cinfo.ifilename); */
        ...
        if (index >= 4) {
            if (overflow_flag == 0) {
                sscanf(line, "%d%s%lf%lf%lf%d%d%d%d%d%d", &tmpint0, atom[i].name,
                       &atom[i].x, &atom[i].y, &atom[i].z, &tmpint1, &tmpint2, &tmpint3,
...
void wcsd(char *filename, int atomnum, ATOM atom[])
{
    ...
    fprintf(fpout, "REFRENCE STRUCTURE = 00000    A,B,C, =   1.000   1.000   1.000\n");
    fprintf(fpout, "ALPHA,BETA,GAMMA = 90.000   90.000   90.000   SPGR =   P1\n");
    fprintf(fpout, "%4d\n", atomnum);
    for (i = 0; i < atomnum; i++) {
        fprintf(fpout, "%4d %-4s%10.5lf%10.5lf%10.5lf", i + 1, atom[i].name, atom[i].x,
                atom[i].y, atom[i].z);
        if (atom[i].con[0] > -1)
