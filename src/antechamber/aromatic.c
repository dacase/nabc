            ...
            if (selectnum == j) {
                for (k = 0; k < 4; k++) {
                    ...
                    if (atm[selectarom[0]].con[k] == start) {
                        for (m = 0; m < j; m++)
                            ring[start][ringindex[start]++] = selectarom[m];
                        ...
                        for (l = 0; l < selectnum; l++)
                            if (atm[selectarom[l]].atomicnum == 6
                                ...
                                aromindex2 = 1;
                        if (atm[start].atomicnum == 6 && atm[start].connum == 4)
                            aromindex2 = 1;
                        if (aromindex2 == 0)
...
/* The following code judge whether an atom is aromatic atom or not */
    ...
    for (i = 0; i < atomnum; i++) {
        if (atom[i].atomicnum != 6 && atom[i].atomicnum != 7 && atom[i].atomicnum != 8
            && atom[i].atomicnum != 16 && atom[i].atomicnum != 15)
            continue;
    ...
    for (i = 0; i < atomnum; i++) {
        if (atom[i].arom == 0)
            ...
            atom[i].arom /= 2;
            fprintf(fptest, "atom[%2d] belongs to one member of %2d aromatic ring\n",
                    i + 1, atom[i].arom);
        }
    }
    fprintf(fptest, "\n              ALIPHATIC RING \n\n");
    for (i = 0; i < atomnum; i++) {
        if (atom[i].aliph == 0)
            ...
            atom[i].aliph /= 2;
            fprintf(fptest, "atom[%2d] belongs to one member of %2d aliphatic ring\n",
                    i + 1, atom[i].aliph);
        }
    }
}
