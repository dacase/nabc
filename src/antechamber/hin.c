...
int rhin(char *filename, int *atomnum, ATOM atom[], int *bondnum, BOND * bond,
         CONTROLINFO cinfo, MOLINFO minfo)
{
    ...
    char tmpchar5[MAXCHAR], tmpchar6[MAXCHAR], tmpchar7[MAXCHAR], tmpchar8[MAXCHAR];
    ...
    for (;;) {
        ...
        if (strncmp("atom", line, 4) == 0) {
            ...
            strcpy(tmpchar11, "");
            tmpchar11[0] = '\0';
            sscanf(line, "%s%d%s%s%s%s%lf%lf%lf%lf%d%d%s%d%s%d%s%d%s%d%s%d%s", tmpchar1,
                   &tmpint1, tmpchar2, tmpchar3, tmpchar4, tmpchar5, &tmpfloat1,
                   &tmpfloat2, &tmpfloat3, &tmpfloat4, &tmpint2, &tmpint3, tmpchar6,
                   &tmpint4, tmpchar7, &tmpint5, tmpchar8, &tmpint6, tmpchar9, &tmpint7,
                   tmpchar10, &tmpint8, tmpchar11);
            if (overflow_flag == 0) {
...
void whin(char *filename, int atomnum, ATOM * atom, int bondnum, BOND * bond)
{
    typedef struct {
        char name[6];
    } ATOMNAME;
...
/*	char tmpchar[MAXCHAR]; */
    ...
    if ((fpout = fopen(filename, "w")) == NULL) {
        fprintf(stdout, "Warning: Cannot open a file (%s) to write.\n", filename);
        return;
    }
/*
...
*/
    ...
    fprintf(fpout, "%s 1 \"%s\"\n", atom[i].aa, filename);
    for (i = 0; i < atomnum; i++) {
        fprintf(fpout, "atom %d - %s ** - %10.5lf%10.5lf%10.5lf%10.5lf %d ", i + 1,
                name[i].name, atom[i].charge, atom[i].x, atom[i].y, atom[i].z,
                atom[i].connum);
