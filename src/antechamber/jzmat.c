...
int rjzmat(char *filename, int *atomnum, ATOM * atom, CONTROLINFO cinfo, MOLINFO minfo)
{
    typedef struct {
        char str[MAXCHAR];
    } STRNAME;
    ...
    *atomnum = numatom;
    ...
    for (;;) {
        ...
        if (overflow_flag == 0 && flag == 1) {
            ...
            tmpchar3[0] = '\0';
            tmpchar4[0] = '\0';
            sscanf(line, "%s%s%s%s%s%s%s", tmpchar1, tmpchar2, bondstr[numatom].str,
                   tmpchar3, anglestr[numatom].str, tmpchar4, twiststr[numatom].str);
            if (numatom > 0) {
                findindex = 0;
                for (i = 0; i < numatom; i++) {
                ...
                if (findindex == 0) {
                    eprintf("The bond atom (%s) does not appear before atom (ID: %d, Name: %s).",
                            tmpchar2, numatom + 1, tmpchar1);
                }
            }
            ...
            if (numatom > 1) {
                ...
                if (findindex == 0) {
                    eprintf("The angle atom (%s) does not appear before atom (ID: %d, Name: %s).",
                            tmpchar3, numatom + 1, tmpchar1);
                }
            }
            ...
            if (numatom > 2) {
                ...
                if (findindex == 0) {
                    eprintf("The torsional angle atom (%s) does not appear"
                            " before atom (ID: %d, Name: %s).",
                            tmpchar4, numatom + 1, tmpchar1);
                }
            }
            numatom++;
...
void wjzmat(char *filename, int atomnum, ATOM atom[], MOLINFO minfo)
{
    ...
    for (i = 0; i < atomnum; i++) {
        ...
        if (i == 1) {
            ...
            fprintf(fpout, "%8s%8s%8s\n", atom[i].name, atom[atom[i].bondatom].name,
                    tmpchar1);
            continue;
        }
        if (i == 2) {
        ...
        strcpy(tmpchar1, "r");
        strcat(tmpchar1, tmpchar0);
        fprintf(fpout, "%8s%8s%8s", atom[i].name, atom[atom[i].bondatom].name, tmpchar1);
        strcpy(tmpchar2, "a");
        strcat(tmpchar2, tmpchar0);
