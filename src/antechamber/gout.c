...
int rgout(char *filename, int *atomnum, ATOM * atom, CONTROLINFO cinfo, MOLINFO * minfo)
{
/*
    ...
    for (;;) {
        ...
        if (fgets(line, MAXCHAR, fpin) == NULL) {
            ...
            if (read_flag == 2 && suc_flag == 0) {
                eprintf("No atom found; the gaussian output file may not be complete.");
            }
            break;
        }
...
/* we prefer to use the first standard orientation after Found_Stationary == 1 */
        ...
        if (scan_flag == 1 && Found_Stationary == 0 && strcmp("--", tmpchar1) == 0
            && strcmp("Stationary", tmpchar2) == 0 && strcmp("point", tmpchar3) == 0
        ...
        if (scan_flag == 0 && (read_flag == 1 || read_flag == 2)) {
            ...
            if (suc_flag == 1 && format == 1 && line[28] != '.' && line[40] != '.'
                && line[52] != '.')
                break;
            if (suc_flag == 1 && format == 2 && line[39] != '.' && line[51] != '.'
                && line[63] != '.')
                break;
        }
    }
...
void wgout()
{
    printf("Warning: Cannot write a gaussian output file.\n"
           "         You must run a Gaussian program (g03, g09, etc.).\n");
}
