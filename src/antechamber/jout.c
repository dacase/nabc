/* Jaguar OUT */
int rjout(char *filename, int *atomnum, ATOM * atom, CONTROLINFO cinfo, MOLINFO * minfo)
{
    int i = 0;
    int index = 0;
...
/*     printf("\nFinished reading %s file.", cinfo.ifilename); */
        ...
        if (strcmp(tmpchar1, "net") == 0 && strcmp(tmpchar2, "molecular") == 0
        ...
        if (read_input_flag == 1 && strcmp(tmpchar1, "Input") == 0
            && strcmp(tmpchar2, "geometry:") == 0) {
            pos = ftell(fpin);
            continue;
        }
