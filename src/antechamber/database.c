...
void calendar(int *year, int *month, int *day, int *hour, int *minute, int *second)
{
...
int calendar_(int *year, int *month, int *day, int *hour, int *minute, int *second)
{
        ...
        if (i < strlen(str) - 6) {
            if (str[i] == 'T' && str[i + 1] == 'M' && str[i + 2] == 'P'
                ...
                if (spaceline(tmp_file) == 1) {
                    eprintf("TMPFILE is null string.");
                }
                strcat(newstr, tmp_file);
                i += 7;
            }
            if (str[i] == 'M' && str[i + 1] == 'O' && str[i + 2] == 'L'
                && str[i + 3] == 'N' && str[i + 4] == 'A' && str[i + 5] == 'M'
                && str[i + 6] == 'E') {
                if (spaceline(molname) == 1) {
                    eprintf("MOLNAME is null string.");
                }
...
void rdef(char *filename)
{
    ...
    for (;;) {
        ...
        if (strncmp("OMIT_LINE", line, 9) == 0) {
            ...
            num_omit++;
            if (num_omit > MAXLINE) {
                eprintf("The number of omitted lines (%d) exceeded MAXLINE.\n"
                        "Increase MAXLINE in database.c and rebuild.", num_omit);
            }
            continue;
        }
        if (strncmp("ADD_LINE", line, 8) == 0) {
            ...
            num_add++;
            if (num_add > MAXLINE) {
                eprintf("The number of added lines (%d) exceeded MAXLINE.\n"
                        "Increase MAXLINE in database.c and rebuild.", num_add);
            }
            continue;
        }
        if (strncmp("COMMAND", line, 7) == 0) {
            ...
            if (num_command > MAXCOMMAND) {
                eprintf("The number of added lines (%d) exceeded MAXCOMMAND.\n"
                        "Increase MAXCOMMAND in database.c and rebuild.", num_command);
            }
            continue;
        }
    }
    ...
    if (def_index == 0 && molname_field_index == 0 && molname_fl == 0) {
        eprintf("No Molecular Name.");
    }
    if (molname_fl != 0)
        if (molname_ld < 1) {
            eprintf("Molname_ld must >=1 when the first line of each record\n"
...
/*get record name */
                    ...
                    printf("\nProcessing: %-8ld id=%7ld, comp. name = %s", cmpd_num,
                           id - 1, molname);
                    fflush(stdout);
                    fprintf(fpprocess, "\nProcessing: %-8ld id=%7ld, comp. name = %s",
                            cmpd_num, id - 1, molname);
...
/*get record name if molname_field_index == 1 for some cases*/
        if (molname_field_index == 1 && strstr(line, field) != 0)
            if ((molname_flag == 2 && molname_ld > 0)
                || (molname_flag == 3 && molname_ld > 0)) {
                strcpy(linebak5, linebak4);
                strcpy(linebak4, linebak3);
                strcpy(linebak3, linebak2);
...
int main(int argc, char *argv[])
{
    ...
    fperror = efopen("error.log", "w");
    fpprocess = efopen("process.log", "w");
    rdef(dfilename);
    if (proc_prob != -1) {
        randomp = (double *) emalloc(sizeof(double) * random_array_size);
        random_id = 0;
        randomp[0] = 0;
        randomnum(random_array_size);
    }
