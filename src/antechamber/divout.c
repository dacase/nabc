            ...
            MOLINFO * minfo)
{
    ...
    for (;;) {
        ...
        if (flag == 1 && strcmp(tmpchar1, "NUMBER") == 0
            && strcmp("SYMBOL", tmpchar2) == 0 && strcmp("X", tmpchar3) == 0
            && strcmp("Y", tmpchar4) == 0 && strcmp("Z", tmpchar5) == 0) {
            flag = 2;
            continue;
        }
        if (flag == 2) {
...
void wdivout(char *filename, int atomnum, ATOM * atom, CONTROLINFO cinfo, MOLINFO minfo)
{
    char tmpchar[MAXCHAR];
    int status = 0;
