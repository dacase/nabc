...
int rcharmm(char *filename, int *atomnum, ATOM atom[], int *bondnum, BOND bond[],
            CONTROLINFO * cinfo, MOLINFO * minfo)
{
    ...
    int numatom2;
    int numbond;
    char tmpchar[MAXCHAR], tmpchar1[MAXCHAR], tmpchar2[MAXCHAR], tmpchar3[MAXCHAR],
        tmpchar4[MAXCHAR], tmpchar5[MAXCHAR];
    char tmpchar6[MAXCHAR], tmpchar7[MAXCHAR], tmpchar8[MAXCHAR], tmpchar9[MAXCHAR],
        tmpchar10[MAXCHAR];
    char line[MAXCHAR];
    char rtf[MAXCHAR];
    int irtf = 0;
    ...
    for (;;) {
        ...
        if ((irtf == 0)
            && (strcmp(tmpchar1, "Read") == 0 || strcmp(tmpchar1, "READ") == 0
                || strcmp(tmpchar1, "read") == 0) && (strcmp(tmpchar2, "Rtf") == 0
                                                      || strcmp(tmpchar2, "RTF") == 0
                                                      || strcmp(tmpchar2, "rtf") == 0)
            && (strcmp(tmpchar3, "Card") == 0 || strcmp(tmpchar3, "CARD") == 0
                || strcmp(tmpchar3, "card") == 0) && (strcmp(tmpchar4, "Name") == 0
                                                      || strcmp(tmpchar4, "NAME") == 0
                                                      || strcmp(tmpchar4, "name") == 0)) {
            strcpy(rtf, tmpchar5);
            irtf = 1;
            continue;
        ...
        if ((rflag == 0)
            && (strcmp(tmpchar1, "Read") == 0 || strcmp(tmpchar1, "READ") == 0
                || strcmp(tmpchar1, "read") == 0) && (strcmp(tmpchar2, "Coor") == 0
                                                      || strcmp(tmpchar2, "COOR") == 0
                                                      || strcmp(tmpchar2, "coor") == 0)
            && (strcmp(tmpchar3, "Card") == 0 || strcmp(tmpchar3, "CARD") == 0
                || strcmp(tmpchar3, "card") == 0)) {
            rflag = 1;
            continue;
        }
        if (rflag == 1) {
        ...
        if (rflag == 2) {
            sscanf(line, "%d%s%s%s%lf%lf%lf", &atom[numatom].id, atom[numatom].resid,
                   atom[numatom].aa, atom[numatom].name, &atom[numatom].x,
    ...
    for (;;) {
        ...
        tmpchar9[0] = '\0';
        tmpchar10[0] = '\0';
        sscanf(line, "%s%s%s%s%s%s%s%s%s%s%s", tmpchar, tmpchar1, tmpchar2, tmpchar3,
               tmpchar4, tmpchar5, tmpchar6, tmpchar7, tmpchar8, tmpchar9, tmpchar10);
...
/* CHARMM */
void wcharmm(char *filename, char *ifilename, int atomnum, ATOM * atom, int bondnum,
...
/*part1: if intype is not prepi, prepc and ac, judge atom type*/
    if ((strcmp(cinfo.intype, "prepi") != 0 && strcmp(cinfo.intype, "prepc") != 0
         && strcmp(cinfo.intype, "5") != 0 && strcmp(cinfo.intype, "6") != 0
         && strcmp(cinfo.intype, "ac") != 0 && strcmp(cinfo.intype, "1") != 0)
        || cinfo.prediction_index == 1 || cinfo.prediction_index == 3) {
        copied_size = build_exe_path(tmpchar, "atomtype", sizeof tmpchar, 1);
        strncat(tmpchar, " -i ANTECHAMBER_PREP.AC0 -o ANTECHAMBER_PREP.AC" " -p ",
                sizeof tmpchar - copied_size);
        strncat(tmpchar, minfo.atom_type_def, sizeof(tmpchar) - strlen(tmpchar) - 1);
    ...
    strncat(tmpchar, " -i ANTECHAMBER_PREP.AC -f ac -o ", sizeof tmpchar - copied_size);
