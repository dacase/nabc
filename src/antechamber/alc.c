/* ALC */
int ralc(char *filename, int *atomnum, ATOM * atom, int *bondnum, BOND * bond,
...
/*       printf("\nFinished reading %s file.", cinfo.ifilename); */
        ...
        index++;
        if (index == 1) {
            sscanf(line, "%d%s%d%s%d%s", &tmpint1, tmpchar1, &tmpint2, tmpchar2, &tmpint3,
                   tmpchar3);
            continue;
        }
        if (index > 1 && index < tmpint1 + 2) {
            if (overflow_flag == 0)
                sscanf(line, "%d%s%lf%lf%lf%lf", &tmpint3, atom[i].name, &atom[i].x,
                       &atom[i].y, &atom[i].z, &atom[i].charge);
            i++;
        ...
        if (index >= tmpint1 + 2 && index < tmpint1 + tmpint2 + 2) {
            ...
            if (overflow_flag == 0) {
                itype = 0;
                atom[tmpint4 - 1].con[atom[tmpint4 - 1].connum++] = tmpint5 - 1;
                atom[tmpint5 - 1].con[atom[tmpint5 - 1].connum++] = tmpint4 - 1;
                if (strcmp(type, "1") == 0)
                    itype = 1;
                if (strcmp(type, "2") == 0)
                    itype = 2;
            ...
            if (j >= cinfo.maxbond && overflow_flag == 0) {
                printf("Info: The number of atoms (%d) exceeded MAXBOND.\n", j);
                overflow_flag = 1;
            }
        }
    }
...
void walc(char *filename, int atomnum, ATOM * atom, int bondnum, BOND * bond)
{
    typedef struct {
        char name[6];
    } ATOMNAME;
    ...
    if ((fpout = fopen(filename, "w")) == NULL) {
        printf("Warning: Cannot open file (%s) to write.\n",
                filename);
        return;
    }
    ...
    for (i = 0; i < atomnum; i++)
        fprintf(fpout, "%5d %-5s%9.4lf%9.4lf%9.4lf%11.4lf\n", i + 1, name[i].name,
                atom[i].x, atom[i].y, atom[i].z, atom[i].charge);
/*
	for (i = 0; i < atomnum; i++)
