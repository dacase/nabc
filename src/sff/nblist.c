/*
 * nblist.c: implement functions to build the non-bonded list
 * by creating and searching a k-d tree.
 *
...
/*
 * The computtional complexity of building the k-d tree is directly
 * proportional to the number of keys used to build the tree. Hence,
 * including the partial atom order (o) in addition to the atomic
 * coordinates x, y, z and (for 4D) w increases the computational
 * complexity. However, including the partial atom order guarantees
 * a unique "super key", which is conceptually the concatenation of
 * o, x, y, z and (for 4D) w, even if x, y, z, and w are not unique.
 *
 * See the comments to the buildkdtree function regarding partition
 * for < and >=. The physically impossible geometry wherein multiple
 * atoms have all-identical x, y, z and (for 4D) w coordinates would
 * cause the k-d tree to become somwhat imbalanced. Inluding the
 * partial atom order as a key guards against this possibility.
 */

#define SORT_ATOM_NUMBERS

/* Sort the upper and lower triangle pair lists using heap sort. */
...
static
void downheap_pairs(int* a, int const n, int k)
{
...
static
void downheap_index(int* a, int const n, int k, REAL_T* const x,
                    int const p, int const dim)
{
...
static
void heapsort_index(int* a, int n, REAL_T* const x,
                    int const p, int const dim)
{
...
/*
 * Scan the src array in ascending order and compare the p value of
 * each corresponding element of the atomic coordinate array x to the
 * median value.  If the p value is less than the median value, copy
 * the element of the src array into the lower part of the dst array.
 * If the p value is greater than or equal to the median value, copy
 * the element of the src array into the upper part of the dst array.
 * The lower part of the dst array begins with the start index, and the
 * upper part of the dst array begins one element above the middle index.
 * At the end of this scan and copy operation, the dst array will have
 ...
 * of the atomic coordinate array x that correspond to elements of the
 * dst array while those elements are partitioned about the p median.
 *
 * Note: when scanning the src array, skip the element (i.e., the atom
 * number) that equals the middle element because that atom number has
 * been stored at this node of the kd-tree.
 *
 * Calling parameters are as follows:
 *
 * dst - the destination array of atom numbers sorted by x, y, z, w or o
 * src - the source array of atom numbers sorted by x, y, z, w or o
 * prt - the partitioning array of atom numbers sorted by x, y, z, w or o
 * x - atomic coordinate array
 * start - first element of the arrays
 * end - last element of the arrays
 * middle - element of the prt array that specifies the partition coordinate
 * imedian - int median value for use with the ordinal atom number o
 * median - REAL_T median value for use with atomic coordinate x, y, z or w
 * lower - index to last copied element of lower part of the dst array (modified)
 * upper - index to last copied element of upper part of the dst array (modified)
 * x - atomic coordinate array containing x, y, z and (for 4D) w coordinates
 * p - the partition (x, y, z, w or o) on which sorting occurs
 * dim - 3 or 4: dimension of the coordinate space
 */

static
void partition(int* dst, int* src, int* prt, REAL_T* const x,
               int const start, int const end, int const middle,
               int const imedian, REAL_T const median,
               int* lower, int* upper, int const p, int const dim)
{
  int i, mid;
  ...
  *upper = middle;
  ...
  for (i = start; i <= end; i++) {
    if (src[i] != mid) {
      ...
      if (((p == 0) && (src[i] < imedian)) ||
          ((p != 0) && (x[dim * src[i] + p - 1] < median)))
#else
        if (x[dim * src[i] + p] < median)
#endif
        {
          dst[++(*lower)] = src[i];
        } else {
 ...
 *
 * The k-d tree was described by Jon L. Bentley in "Multidimensional
 * Binary Search Trees Used for Associative Searching," Communications
 * of the ACM, 18(9): 509-517, 1975. The algorithm that builds the k-d
 * tree is similar to the algorithms described by Russell A. Brown in
 * "Building a Balanced k-d Tree in O(kn log n) Time," Journal of
 * Computer Graphics Techniques, 4(1): 50-68, 2015.
 *
 * Calling parameters are as follows:
 *
...
void buildkdtree(int* xn, int* yn, int* zn, int* wn, int* on, int *tn,
                 int const start, int const end, KDNODE_T** kdpptr,
                 KDNODE_T* const that, REAL_T* const x,
                 int const p, int const dim)
{
  int i, r, middle, imedian, lower, upper;
  REAL_T median;
  ...
  /*
   * Otherwise, if two elements are passed to this function,
   * check whether the first element is < the second element.
   * If so, assign the first element to the < subtree; else,
   * assign the first element to the >= subtree. The subtree
   * comprises a single KDNODE_T from the kdpptr array.
   */
  ...
  /* Otherwise, more than two elements are passed to this function. */
     ...
     * which the yn and zn arrays will be partitioned.  Avoid overflow
     * when calculating the middle element because a simple average of
     * the start and end elements may cause overflow.
     *
     * Search lower elements of the xn array to ensure that the p values
     * of the atomic coordinate array that correspond to these elements
     * are indeed less than the median value because they may equal it.
     * This approach partitions for < and >=. NOTE that in the case where
     * all coordinates for a given p are equal, all elements except the
     * partitioning element will be assigned to the >= subtree.
     *
    ...
    partition(tn, yn, xn, x, start, end, middle, imedian, median, &lower, &upper, p, dim);
    ...
    partition(yn, zn, xn, x, start, end, middle, imedian, median, &lower, &upper, p, dim);

    /*
     * Execute the following region of code if SORT_ATOM_NUMBERS is defined,
     * or if SORT_ATOM_NUMBERS is undefined and dim==4.
     */
...

    {
       ...
       * This approach preserves the relative heapsorted order of elements
       * of the atomic coordinate array that correspond to elements of the
       * wn array (if SORT_ATOM_NUMBERS is undefined and dim==4) or of the
       * on array (if SORT_ATOM_NUMBERS is defined and dim==3) while those
       * elements are partitioned about the p median.
       */

      partition(zn, wn, xn, x, start, end, middle, imedian, median, &lower, &upper, p, dim);
    }
    ...
    if (dim == 4) {
       ...
       * This approach preserves the relative heapsorted order of elements
       * of the atomic coordinate array that correspond to elements of the
       * on array while those elements are partitioned about the p median.
       */

      partition(wn, on, xn, x, start, end, middle, imedian, median, &lower, &upper, p, dim);
    }
#endif

    /*
     * Calculate the partition r for the recursive call to this function.
     ...
     * coordinate. The correspondence between partition values and
     * atom number or cartesian coordinate is defined by the order of
     * the calling parameters provided to this buiildkdtree function
     * by the nblist function. NOTE that although the partition cycles
     * by dim+1 when SORT_ATOM_NUMBERS is defined, it nevertheless cycles
     * by dim for the purpose of atomic coordinate selection.
     */

#ifndef SORT_ATOM_NUMBERS
    r = p + 1;
    // less computation than modulus operation for r < 2 * dim
    r = (r < dim) ? r : r - dim;
#else
    r = p + 1;
    // less computation than modulus operation for r < 2 * (dim + 1);
    r = (r < dim + 1) ? r : r - dim - 1;
#endif
    ...
    if (lower >= start) {
      ...
      if (dim == 4) {
        buildkdtree(tn, yn, zn, xn, on, wn,
                    start, lower, kdpptr, that->lo, x, r, dim);
      } else {
    ...
    if (upper > middle) {
      ...
      if (dim == 4) {
        buildkdtree(tn, yn, zn, xn, on, wn,
                    middle + 1, end, kdpptr, that->hi, x, r, dim);
      } else {
        buildkdtree(tn, yn, xn, wn, on, zn,
                    middle + 1, end, kdpptr, that->hi, x, r, dim);
      }
...
static
void searchkdtree(KDNODE_T* const that, REAL_T* const x, INT_T const p,
                  INT_T const q, INT_T* loindexp, INT_T* upindexp,
                  INT_T* lopairlist, INT_T* uppairlist,
                  REAL_T const cut, REAL_T const cut2,
                  int const dim, int* const frozen)
{
  INT_T r;
...
#endif
    {
      searchkdtree(that->hi, x, r, q, loindexp, upindexp,
...
#endif
    {
      searchkdtree(that->lo, x, r, q, loindexp, upindexp,
                   lopairlist, uppairlist, cut, cut2, dim, frozen);
    }
}
...
INT_T nblist(INT_T* lpears, INT_T* upears, INT_T** pearlist, REAL_T* const x,
             INT_T const context_PxQ, INT_T const derivs, REAL_T const cutoff,
             int const natom, int const dim, int* const frozen)
{
...
#else
  int *xn, *yn, *zn, *wn = NULL, *on = NULL, *tn, *lopairlist, *uppairlist;
#endif
  ...
  buildkdtree(xn, yn, zn, wn, on, tn, 0, natom-1, &kdptr, root, x, 0, dim);

#else

  buildkdtree(on, xn, yn, zn, wn, tn, 0, natom-1, &kdptr, root, x, 0, dim);
