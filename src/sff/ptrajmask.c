...
#  include "omp.h"
#endif
#include "sff.h"   /* was originally ptrajmask.h */
// -----------------------------------------------------------------------------
 ...
 * 
 * DAC change, 1/18: return an int array with 0 or 1 for selection; this
 *   is for use in sff. Also, get most info from a PARMSTRUCT_T
 */
int *parseMaskString(char *maskstr, PARMSTRUCT_T *prm, double *X, int debug) 
{
  ...
  mask = eval(postfix, prm->Natom, prm->Nres, prm->AtomNames, 
        prm->ResNames, prm->Ipres, X, prm->AtomType);

  imask = malloc( sizeof(int) * prm->Natom );  /* no error checks in this file! */
  int i;
  for( i = 0; i<prm->Natom; i++ ) imask[i] = mask[i]=='T' ? 1 : 0;
  free(mask);
