...
int main( int argc, char *argv[] )
{
	...
	//first, sort out msanderhome
	if(getenv("MSANDERHOME") == NULL )
	{
	   std::cerr << "error: MSANDERHOME is not set!" << std::endl;
	   exit(1);
	}
	msanderhome = getenv("MSANDERHOME");
...
static void compileNab(std::vector<std::string> inputFileBasenames, std::vector<std::string> cppArgs)
{
	...
	for(std::vector<std::string>::iterator inputFileBasename = inputFileBasenames.begin(); inputFileBasename != inputFileBasenames.end(); ++inputFileBasename)
	{
		...
		cppCmd.arg("-I").arg(msanderhome + "/include").arg(inputFile);
		...
		// next run nab2c 
#ifdef MPI
		CommandLine nabCmd(msanderhome + "/bin/mpinab2c");
#else
		CommandLine nabCmd(msanderhome + "/bin/nab2c");
#endif
...
static void linkC(std::string linker, std::vector<std::string> objectFiles, std::string outputFilename)
{
	...
	cmd.arg("-L" + msanderhome + "/lib");

	// set up RPATH
#if defined(__APPLE__) || defined(__linux__)
	// On OS X, Amber libraries use '@rpath/<name>.dylib` as their
	// install_name, so we have to add $MSANDERHOME/lib as the RPATH
	// On Linux, we can still fall back to LD_LIBRARY_PATH, but setting the RPATH makes it a bit more convenient for the user.
	cmd.arg("-Wl,-rpath," + msanderhome + "/lib");
#endif
