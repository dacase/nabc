...
char	*argv[];
{
	...
	//sprintf( symfname, "%s/../%s", DATDIR,  );
	if( ( fp = fopen( SYM_FILE, "r" ) ) == NULL ){
...

static	int	hprint_rule(char* rulePath, RULE_T * rp)
{
	int	err;
	...
	char* rnp = ruleFilename; //if this is put inside the for loop on the next line, the Intel compiler derps out and can't compile it
	for(; *rnp; rnp++ ){ //attempt to reterminate ruleFilename at the .
		if( *rnp == '.' )
		{
			*rnp = '\0';
		}
	...
	getcwd((char*)&workingDir, 256);

	sprintf(htmlFilename, "%s/%s.html", workingDir, ruleFilename);
